pipeline {
    ...
    stages {
        ...
        stage('Creación de Bucket en GCP') {
            steps {
                echo 'Iniciando la creación de Bucket en GCP...'
                ...
                if (bucketExists != 0) {
                    ...
                    echo "El bucket gs://${NAME_BUCKET_GCP} ya existe."
                }
            }
        }
        ...
        stage('Creación de transferencia de datos de AWS a GCP') {
            steps {
                script {
                    echo 'Iniciando la creación de transferencia de datos de AWS a GCP...'
                    ...
                    // Verifica si el trabajo de transferencia ya existe
                    ...
                    if (existingJob == 0) {
                        ...
                        command = "gcloud transfer jobs update ${NAME_TRANSFER}"
                    } else {
                        // Si el trabajo no existe, crearlo
                        command = "gcloud transfer jobs create s3://${NAME_BUCKET_S3} gs://${NAME_BUCKET_GCP} --name=${NAME_TRANSFER}"
                    }
                    
                    // Ejecutar el comando de creación o actualización
                    sh """
                        ${command} \
                    ...
                    echo 'Transferencia creada o actualizada correctamente.'
                }
            }
        }
